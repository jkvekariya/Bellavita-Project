...
const OrderDetail = () => {
    ...
    return (
        <div className="p-2 md:p-6 max-w-7xl mx-auto min-h-screen bg-gray-50">
            <div className="flex flex-col md:flex-row justify-between items-center mb-4 md:mb-8 gap-4">
                ...
                <div className="flex w-full md:w-auto gap-2 md:gap-4 mt-2 md:mt-0">
                    <div className="flex-1 bg-white p-3 md:p-4 rounded-lg shadow-sm border border-gray-100 text-center min-w-[100px] md:min-w-[120px]">
                        <p className="text-gray-500 text-[10px] md:text-xs uppercase">Total Orders</p>
                        <p className="text-lg md:text-2xl font-bold text-gray-900">{orders.length}</p>
                    </div>
                    <div className="flex-1 bg-white p-3 md:p-4 rounded-lg shadow-sm border border-gray-100 text-center min-w-[100px] md:min-w-[120px]">
                        <p className="text-gray-500 text-[10px] md:text-xs uppercase">Pending</p>
            ...
            {/* Filters */}
            <div className="bg-white p-3 md:p-4 rounded-lg shadow-sm border border-gray-200 mb-4 md:mb-6 flex flex-col md:flex-row gap-3 md:gap-4 items-center">
                <div className="relative flex-1 w-full">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
                    <input
                        type="text"
                        placeholder="Search by Order ID, Name..."
                        className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-black text-sm md:text-base"
                        value={searchTerm}
                ...
                <div className="flex flex-col sm:flex-row gap-3 md:gap-4 w-full md:w-auto">
                    <select
                        className="w-full sm:w-auto px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-black bg-white text-sm md:text-base"
                        value={statusFilter}
                    ...
                    <input
                        type="date"
                        className="w-full sm:w-auto px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-black text-sm md:text-base"
                        value={dateFilter}
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                <div className="overflow-x-auto min-w-full">
                    <table className="w-full text-sm text-left whitespace-nowrap min-w-max">
                        <thead className="bg-gray-50 text-gray-600 uppercase text-[10px] md:text-xs font-semibold border-b border-gray-200">
                            <tr>
                                <th className="px-3 py-3 md:px-6 md:py-4">Order ID</th>
                                <th className="px-3 py-3 md:px-6 md:py-4">Customer</th>
                                <th className="px-3 py-3 md:px-6 md:py-4">Payment</th>
                                <th className="px-3 py-3 md:px-6 md:py-4">Status</th>
                                <th className="px-3 py-3 md:px-6 md:py-4">Date</th>
                                <th className="px-3 py-3 md:px-6 md:py-4 text-center">Actions</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100">
                            {filteredOrders.length > 0 ? filteredOrders.map((order) => (
                                <tr key={order._id} className="hover:bg-gray-50 transition-colors text-xs md:text-sm">
                                    <td className="px-3 py-3 md:px-6 md:py-4 font-medium text-gray-900">
                                        {order.orderId || `#${order._id.slice(-6).toUpperCase()}`}
                                        <div className="text-[10px] md:text-xs text-gray-500 mt-0.5 md:mt-1">{order.orderType || 'Online'}</div>
                                    </td>
                                    <td className="px-3 py-3 md:px-6 md:py-4">
                                        <div className="font-medium text-gray-900 truncate max-w-[120px] md:max-w-[200px]">{order.fullName}</div>
                                        <div className="text-[10px] md:text-xs text-gray-500">{order.mobileNumber}</div>
                                    </td>
                                    <td className="px-3 py-3 md:px-6 md:py-4">
                                        <div className="font-bold">₹{order.totalAmount}</div>
                                        <div className="text-[10px] md:text-xs text-gray-500 mt-0.5 md:mt-1 flex items-center gap-1">
                                            {order.paymentMethod || 'Online'}
                                            <span className={`px-1.5 py-0.5 rounded text-[9px] md:text-[10px] ${order.paymentStatus === 'Paid' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                                                {order.paymentStatus || 'Pending'}
                                            </span>
                                        </div>
                                    </td>
                                    <td className="px-3 py-3 md:px-6 md:py-4">
                                        <span className={`px-2 py-0.5 md:px-3 md:py-1 rounded-full text-[10px] md:text-xs font-medium ${statusColors[order.status] || 'bg-gray-100 text-gray-800'}`}>
                                            {order.status}
                                        </span>
                                    </td>
                                    ...
                                    <td className="px-3 py-3 md:px-6 md:py-4">
                                        <div className="flex justify-center gap-1 md:gap-2">
                                            <button
                                                ...
                                                className="p-1.5 md:p-2 hover:bg-gray-100 rounded-full text-blue-600 transition-colors"
                                                title="View Details"
                                            >
                                            ...
                                            <button
                                                ...
                                                className="p-1.5 md:p-2 hover:bg-gray-100 rounded-full text-gray-600 transition-colors"
                    ...
                    <div className="bg-white rounded-xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                        <div className="p-4 md:p-6 border-b border-gray-100 flex justify-between items-center sticky top-0 bg-white z-10">
                            <div>
                                ...
                                <p className="text-xs md:text-sm text-gray-500">ID: {selectedOrder.orderId || selectedOrder._id}</p>
                            </div>
                            <button onClick={() => setSelectedOrder(null)} className="p-1.5 md:p-2 hover:bg-gray-100 rounded-full">
                                <X size={20} />
                            </button>
                        </div>
